...
import { MealCardsSkeleton } from "@/app/(dashboard)/client/_components/meal-cards-skeketon";
import { useMealStore } from "@/app/(dashboard)/client/_lib/use-meal-store";
...
const MealCards = () => {
  ...
  const calculateTotalCalories = (mealFoods: MealWithFoods["MealFood"]) => {
  ...
  const calculateNutritionTotals = (meals: MealWithFoods[]) => {
    return (
      meals?.reduce(
        (totals, meal) => {
          meal.MealFood.forEach((mealFood) => {
            const multiplier = mealFood.amount || 0;
            ...
            totals.sugar += (mealFood.food.sugars || 0) * multiplier;
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {mealsQuery.data?.map((meal) => {
            const totalCalories = calculateTotalCalories(
              meal.MealFood as MealWithFoods["MealFood"]
            );
